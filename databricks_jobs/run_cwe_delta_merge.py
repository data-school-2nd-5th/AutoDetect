...
if base_path.startswith("/Workspace"):
    repo_root = base_path
else:
    repo_root = f"/Workspace{base_path}"
...
def patched_read_xml_text(spark, source_xml_path):
    normalized_path = normalize_path(source_xml_path)
    scheme = urlparse(normalized_path).scheme.lower()
    if scheme in {"http", "https"}:
        with urlopen(normalized_path, timeout=60) as response:
