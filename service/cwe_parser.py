...
def parse_cwe_weaknesses(xml_content: str | bytes) -> List[Dict[str, str]]:
    ...
    for weakness in root.findall(".//Weakness"):
        modification_dates = [
            ...
            for modified in weakness.findall("./Content_History/Modification/Modification_Date")
        ...
        row = {
            ...
            "demonstrative_examples": _json_string(_parse_demonstrative_examples(weakness)),
            "selected_observed_examples": _json_string(_parse_observed_examples(weakness)),
...
def _parse_demonstrative_examples(weakness: ET.Element) -> List[Dict[str, str]]:
    ...
    for example in weakness.findall("./Demonstrative_Examples/Demonstrative_Example"):
        ...
        body_texts = [
            _node_text(node)
            for node in example.findall("Body_Text")
            if _node_text(node)
        ]
        examples.append(
            {
